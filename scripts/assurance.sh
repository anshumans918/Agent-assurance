...
# invents. The workflow provisions the real names (credentials included) before
# this script runs, so a stub here is a name test-data/openemr-portal.json does
# not supply yet — the evidence job's preflight will stop on it.
python3 scripts/test_data.py stubs || true
